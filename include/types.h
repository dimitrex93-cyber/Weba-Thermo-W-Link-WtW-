...
struct RTCData {
  ...
  uint32_t letzteLogZeit;            // 4 bytes periodic log cadence anchor
  uint32_t unixBaseTime;             // 4 bytes unix timestamp at sync moment
  uint32_t unixBaseTick;             // 4 bytes logical tick at sync moment
  uint32_t startTick;                // 4 bytes logical time across sleep cycles
